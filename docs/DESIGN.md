...
| **M1** faithful core (done; reporters — GitLab Code Quality, SARIF, GitHub annotations — with file/line locations from kustomize origin tracking added later; differential tests against `fluxcd/pkg/kustomize` — the generator and SecureBuild kustomize-controller uses — are in `pkg/render`) | in-process kustomize, Flux post-processing, config + externals, `FL-G001–5/8`, `FL-S001/2/4`, `FL-T001/2/4–8/100`, text + JSON reports, synthetic fixtures |
...
| **M2c** admission (done) | custom resources against the rendered CRDs — served versions (`FL-G006`) and schema after structural defaulting via the apiextensions validator (`FL-V001`); Pod Security via `k8s.io/pod-security-admission` (`FL-V002`). CEL `x-kubernetes-validations` are evaluated with the apiextensions CEL validator; built-in kinds are decoded strictly into their Go types (`FL-V003`: unknown fields, wrong types — what server-side apply rejects). Value-level validation of built-ins (the API server's internal validation package) is out of reach without a cluster |
...
| **M6b** contracts (done: `fluxlint-contract.yaml` in external sources, `FL-C001`, `FL-R007` go.mod skew, RBAC as dependency justification; see CONTRACTS.md; charts can ship one beside Chart.yaml) | contract format, envtest recipe for operator authors, CEL assertions |
